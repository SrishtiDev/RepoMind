...
function getLLM() {
    if (!process.env.GROQ_API_KEY) {
        throw new Error("[Answer] GROQ_API_KEY is not set. Cannot initialise Groq.");
    }
...
async function answerNode(state) {
    if (state.retrievedChunks.length === 0) {
        ...
        return {
            answer: "I couldn't find relevant information in this repo to answer your question.",
            sources: [],
        };
    }
    const llm = getLLM();
...
QUESTION: ${state.question}`;
    ...
    try {
        const response = await invokeWithRetry(llm, [
            new messages_1.SystemMessage("You are a grounded code assistant. Never hallucinate. Always cite sources."),
            new messages_1.HumanMessage(answerPrompt),
        ], "Answer");
        rawAnswer = String(response.content).trim();
    }
    catch (err) {
        throw new Error(`[Answer] Groq generation failed: ${err?.message ?? err}`);
    }
    ...
    const sources = state.retrievedChunks.map((c) => ({
        filename: c.filename,
        filepath: c.filepath,
        chunkIndex: c.chunkIndex,
    }));
    console.log(`[Answer] Generated answer (${rawAnswer.length} chars) with ${sources.length} source(s).`);
