 ...
 */
export function buildGraph(files: FileData[]): DirectedGraph {
  const graph = new DirectedGraph({ multi: false, allowSelfLoops: true });

  const fileExports = new Map<string, Map<string, string>>(); // filepath -> (functionName -> nodeKey)
  const fileFunctions = new Map<string, Map<string, string>>(); // filepath -> (functionName -> nodeKey)

  // First pass: Create nodes and 'contains' edges
  for (const file of files) {
    graph.addNode(file.filepath, { type: "file" });

    if (!fileExports.has(file.filepath)) fileExports.set(file.filepath, new Map());
    if (!fileFunctions.has(file.filepath)) fileFunctions.set(file.filepath, new Map());
  ...
  const resolveImportPath = (sourceFilepath: string, importSource: string): string | null => {
    if (!importSource.startsWith(".")) return null;
    const dir = path.dirname(sourceFilepath);
    const resolvedPathBase = path.normalize(path.join(dir, importSource));

    for (const ext of ["", ".ts", ".tsx", ".js", ".jsx", "/index.ts", "/index.js"]) {
      const tryPath = resolvedPathBase + ext;
      if (files.some((f) => f.filepath === tryPath)) return tryPath;
    }
    return null;
  };
  ...
  for (const file of files) {
    const importResolution = new Map<string, string>(); // imported name -> target filepath
    ...
    for (const call of file.calls) {
      ...
      if (!callerKey) continue; // Should not happen since we extract calls within functions
