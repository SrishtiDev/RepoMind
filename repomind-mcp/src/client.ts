...
const BASE_URL =
  (process.env.REPOMIND_API_URL ?? "http://localhost:3000").replace(/\/$/, "");
...
async function post<TBody, TResponse>(
  ...
  try {
    ...
    throw new Error(
      `RepoMind backend is unreachable at ${BASE_URL}. ` +
        `Is the server running? (${(err as Error).message})`
    );
  }
...
export async function ingestRepo(repoUrl: string): Promise<IngestResponse> {
  return post<{ repoUrl: string }, IngestResponse>("/ingest", { repoUrl });
}
...
export async function askQuestion(question: string): Promise<QueryResponse> {
  return post<{ question: string }, QueryResponse>("/query", { question });
}
