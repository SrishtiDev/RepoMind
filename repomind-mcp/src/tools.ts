...
import { z } from "zod";
import { ingestRepo, askQuestion, type Source } from "./client.js";
...
export const INGEST_TOOL_NAME = "repomind_ingest" as const;
...
export const ingestToolSchema = z.object({
  repoUrl: z
    ...
    .regex(
      GITHUB_URL_RE,
      "repoUrl must be a valid GitHub URL, e.g. https://github.com/owner/repo"
    ),
});
...
export const ingestZodShape = {
  repoUrl: z
    ...
    .describe(
      "Full GitHub repository URL to ingest, e.g. https://github.com/owner/repo"
    ),
};

export const INGEST_TOOL_DESCRIPTION =
  "Ingest a public GitHub repository into RepoMind so it can be queried. " +
 ...
 */
export async function handleIngest(rawInput: unknown): Promise<string> {
...
export const ASK_TOOL_NAME = "repomind_ask" as const;
...
export const askZodShape = {
  question: z
    ...
    .describe(
      "A natural-language question about the ingested GitHub repository's code or architecture."
    ),
};

export const ASK_TOOL_DESCRIPTION =
  "Ask a question about a previously ingested GitHub repository. " +
...
function formatSources(sources: Source[]): string {
  if (sources.length === 0) return "  (no source citations available)";

  return sources
    .map(
      (s, idx) =>
        `  [${idx + 1}] ${s.filename}  →  ${s.filepath}  (chunk ${s.chunkIndex})`
    )
    .join("\n");
}
 ...
 */
export async function handleAsk(rawInput: unknown): Promise<string> {
